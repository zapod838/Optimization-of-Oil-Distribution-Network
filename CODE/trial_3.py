...
for tanker_type in tankers_capicity_in_barrels:
    ...
    for tanker in tankers_capicity_in_barrels[tanker_type]:
        for destination in destinations:
            ...
            crude_oil_cost = crude_oil_costs_per_barrel[crude_type] * QUANTITY_PER_TYPE_PER_DESTINATION
            total_cost = rate + port_charge + fuel_cost + crude_oil_cost
            objective.append((var_name, total_cost))
            # Print statement for debugging
            print(f"Route: {var_name}, Total Cost: {total_cost}")
...
# Initialize a set to track allocated boats
allocated_boats = set()

sorted_boats_by_route = {}
for port in ports:
    for destination in destinations:
        # Filter boats that are not already allocated and have sufficient capacity
        eligible_boats = [(var_name, cost) for var_name, (cost, capacity) in costs_and_capacities.items()
                          if var_name.split('_')[1] not in allocated_boats and capacity >= QUANTITY_PER_TYPE_PER_DESTINATION and var_name.startswith(port + "_")]
...
# Constraint: Each port must send to each destination exactly once
for port in ports:
    for destination in destinations:
...
# After setting up the model and constraints
                ...
                for destination in destinations:
                    ...
                    try:
                        if solution.get_values(var_name) > 0.5:
                            print(f"From {port}, Tanker {tanker} to {destination}")
                            found_solution = True
